...
const TYPE_COLOR: Record<InstitutionType, string> = {
  ...
  police:
    "from-slate-500/20 to-slate-600/10 border-slate-500/20 text-slate-300",
  blood_bank:
        ...
        return (
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              <Icon
                className={`w-4 h-4 ${isActive ? "text-terracotta-500" : "text-slate-400"}`}
              />
              <span
                className={`text-[10px] font-semibold uppercase tracking-wider truncate ${isActive ? "text-slate-800" : "text-slate-500"}`}
              >
                {cat.label.split(" ")[0]}
              </span>
            </div>
