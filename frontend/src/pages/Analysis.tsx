      ...
      <div className="glass-card p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h3 className="text-lg font-semibold text-white">
            Forecast Model ({regionId} - {disease})
          </h3>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {evaluation ? (
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="p-4 rounded-lg bg-white/5 border border-white/10 text-center">
                  <div className="text-2xl font-bold text-white mb-1">
                    {evaluation.aggregate_mape
                      ? `${(evaluation.aggregate_mape * 100).toFixed(1)}%`
                      : "N/A"}
                  </div>
                  <div className="text-xs text-gray-400 uppercase tracking-wider">
                    MAPE
        ...
        <div className="glass-card p-6">
          ...
          <p className="text-gray-400 text-sm leading-relaxed">
            Based on current trends, {disease} cases in {regionId} are expected
            to
            {forecasts?.forecasts &&
            forecasts.forecasts.length > 1 &&
            forecasts.forecasts[forecasts.forecasts.length - 1].pred_mean >
              forecasts.forecasts[0].pred_mean
              ? " increase "
              : " decrease "}
            over the next week. Resource allocation should validate the stock of
            critical supplies.
          </p>
        </div>
      </div>
