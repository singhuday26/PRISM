        ...
        <div className="space-y-8 max-w-2xl">
            ...
            <div className="glass-card p-8">
                ...
                <div className="space-y-6">
                    ...
                    {/* Risk Threshold */}
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-gray-300">Minimum Risk Level</label>
                        <div className="grid grid-cols-3 gap-2">
                            {['MEDIUM', 'HIGH', 'CRITICAL'].map((level) => (
                                <button
                                    key={level}
                                    onClick={() => setMinRisk(level)}
                                    className={`px-4 py-2 rounded-lg text-sm font-medium border transition-all ${minRisk === level
                    ...
                    {/* Diseases */}
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-gray-300">Monitored Diseases</label>
                        <div className="space-y-2">
                            {['DENGUE', 'MALARIA', 'CHIKUNGUNYA'].map((d) => (
                                <label key={d} className="flex items-center gap-3 p-3 rounded-lg bg-white/5 border border-white/5 cursor-pointer hover:border-white/20 transition-colors">
                                    <input
                                        type="checkbox"
                                    ...
                                    />
                                    <span className="text-white capitalize">{d.toLowerCase()}</span>
                                </label>
                            ))}
                        </div>
                    </div>
